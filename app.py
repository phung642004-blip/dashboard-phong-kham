...
with tab_admin:
    ...
    # CHIA LẠI LÀM 3 CỘT (Cột giữa rộng hơn để chứa 2 phòng khám)
    ...
    with col_bacsi:
        ...
        def render_room_control(ten_phong, col_container):
            with col_container:
                ...
                # Gom cả người chờ mới và người bị qua lượt quay lại
                ds_cho = df[df['Trang_thai'].isin(['Chờ khám', 'Qua lượt'])]
                if not ds_cho.empty:
                    # Tạo nhãn hiển thị dễ nhìn cho bác sĩ (VD: [Chờ khám] 001 - 2501 (2D))
                    ds_cho['Nhan_hien_thi'] = "[" + ds_cho['Trang_thai'] + "] " + ds_cho['STT'] + " - " + ds_cho['PID'] + " (" + ds_cho['Loai_sieu_am'] + ")"
                    ...
                    if st.button(f"📢 Gọi vào {ten_phong}", key=f"call_{ten_phong}"):
                        # Trích xuất số thứ tự từ chuỗi hiển thị
                        stt_chon = bn_chon.split("] ")[1].split(" - ")[0]
                        
                        # Nếu phòng đang có người, tự động đẩy người cũ ra chờ kết quả
                        df.loc[(df['Trang_thai'] == 'Đang phục vụ') & (df['Phong'] == ten_phong), 'Trang_thai'] = 'Chờ trả kết quả'
                        
                        # Đưa người mới vào phòng
